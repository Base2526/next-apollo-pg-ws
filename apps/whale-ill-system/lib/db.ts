...
const whaleDbName = process.env.WHALE_DB_NAME || "whale_ill_system";

if (
  process.env.NODE_ENV !== "test" &&
  process.env.POSTGRES_DB &&
  process.env.POSTGRES_DB === whaleDbName
) {
  throw new Error("WHALE_DB_NAME must be isolated from POSTGRES_DB");
}

const pool = new Pool({
  host: process.env.WHALE_DB_HOST || process.env.POSTGRES_HOST || "localhost",
  port: Number(process.env.WHALE_DB_PORT || process.env.POSTGRES_PORT || 5432),
  database: whaleDbName,
  user: process.env.WHALE_DB_USER || process.env.POSTGRES_USER || "app",
  password: process.env.WHALE_DB_PASSWORD || process.env.POSTGRES_PASSWORD || "app",
  max: Number(process.env.WHALE_DB_POOL_MAX || 20),
});
...
export async function ensureWhaleSchema(): Promise<void> {
...
export function getWhaleDatabaseName(): string {
  return whaleDbName;
}
