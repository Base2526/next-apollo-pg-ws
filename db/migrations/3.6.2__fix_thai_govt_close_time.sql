-- Fix Thai Government Lottery close times
-- Business rule: close at 15:30 Bangkok time on draw_date

-- Update function to use correct close time
...
BEGIN
  ...
  -- Close at 15:30 Bangkok time on draw_date
  ...
  INSERT INTO lotto_draws (
    ...
    v_first_date - INTERVAL '7 days', v_first_close_at,
    'DRAFT', true
  ...
  INSERT INTO lotto_draws (
    ...
    v_second_date - INTERVAL '7 days', v_second_close_at,
    'DRAFT', true
  ) ON CONFLICT (code) DO UPDATE SET
    draw_date = CASE WHEN lotto_draws.is_date_overridden = false THEN EXCLUDED.draw_date ELSE lotto_draws.draw_date END,
    name_th = CASE WHEN lotto_draws.is_date_overridden = false THEN EXCLUDED.name_th ELSE lotto_draws.name_th END,
...
-- Fix existing draws that don't have is_date_overridden or have wrong close_at
-- Update close_at for all Thai Government draws to 15:30 Bangkok time on their draw_date
...
-- Re-generate draws to ensure all have correct times
DO $$
DECLARE
  v_year INTEGER := 2026;
  v_month INTEGER := 5;
